...
/*SCROLL CONTACT*/
  ...
  function typeText() {
    if (currentIndex < texts.length && currentText.length < texts[currentIndex].length) {
      currentText += texts[currentIndex].charAt(currentText.length);
      if (textAnimation) {
        textAnimation.textContent = currentText;
      }
  ...
  function removeText() {
    if (currentText.length > 0) {
      ...
      setTimeout(removeText, 50); // Delay between removing each character
    } else {
      currentIndex++;
